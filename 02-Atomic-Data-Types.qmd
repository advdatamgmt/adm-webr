...
Useful operators for the logical data type include `&`{.r}, `|`{.r}, and `!`{.r}. They are called logical operators. The `|`{.r} is often called a pipe character. You usually find it above the backslash <kbd>`\`</kbd> key on the right side of your keyboard. Let's try each of the logical operators out, starting with the `!`{.r} operator. In the code cell below, try evaluating `!TRUE`{.r} and `!FALSE`{.r}, then answer the questions that follow.
